...
module.exports = class OAuth2
  ...
  start : ()->
    ...
    usr = @deps.usr
    return (req, res, next)=>
      console.log "OAUTH2 Authentification start ?"
      if req.session[_.SESSION_NAME]?
        req.oauth2 = req.session[_.SESSION_NAME]
        return next()
      # TODO ?? Improve next test with all params ?? is it necessary ?
      if not req.param('client_id')? or not req.param('redirect_uri')?
        return next()
      Q.all([
        Q.all([
